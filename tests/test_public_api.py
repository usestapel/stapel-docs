...
class TestLazyExports:
    def test_all_declares_public_api(self):
        assert stapel_docs.__all__ == [
            "docs_settings",
        ]

    def test_settings_resolve(self):
        from stapel_docs.conf import docs_settings

        assert stapel_docs.docs_settings is docs_settings

    def test_unknown_attribute_raises(self):
        try:
            stapel_docs.nonexistent_export
...
class TestImportWithoutDjangoSettings:
    def test_package_import_is_django_free(self):
        """`import stapel_docs` must not import Django nor require settings."""
