...
Resolution order per key: `settings.STAPEL_DOCS[key]` -> flat Django setting
...
|---|---|---|---|
| Function (provides) | `docs.ping` | `{}` -> `{greeting}` | `schemas/functions/docs.ping.json` |
...
`stapel_docs/` — it's upstream. If a setting, subclass, receiver or comm
